...
public class CrossInfoTest extends ComponentTestCase {
	...
	public void testParsePigeonClientTransaction() throws Exception {
		...
		DefaultTransaction t = new DefaultTransaction("DubboCall", "method1", null);
		MessageTree tree = buildMockMessageTree();
		CrossInfo info = analyzer.parseCrossTransaction(t, tree);
		...
		Message message = new DefaultEvent("DubboCall.server", "10.1.1.1", null);
		Message messageApp = new DefaultEvent("DubboCall.app", "myDomain", null);
		t.addChild(message);
		...
		Assert.assertEquals(info.getDetailType(), "DubboCall");
		Assert.assertEquals(info.getRemoteRole(), "Dubbo.Server");
	...
	public void testParsePigeonServerTransaction() throws Exception {
		...
		DefaultTransaction t = new DefaultTransaction("DubboService", "method1", null);
		MessageTree tree = buildMockMessageTree();
		CrossInfo info = analyzer.parseCrossTransaction(t, tree);

		Assert.assertEquals(info.validate(), false);

		Message message = new DefaultEvent("DubboService.client", "192.168.7.71", null);
		Message messageApp = new DefaultEvent("DubboService.app", "myDomain", null);
		t.addChild(message);
		t.addChild(messageApp);
		...
		Assert.assertEquals(info.getDetailType(), "DubboService");
		Assert.assertEquals(info.getRemoteRole(), "Dubbo.Client");
		Assert.assertEquals(info.getApp(), "myDomain");
	}
	...
	public void testParsePigeonServerTransactionWithPort() throws Exception {
		...
		Message message = new DefaultEvent("DubboService.client", "192.168.7.71:29987", null);
		Message messageApp = new DefaultEvent("DubboService.app", "myDomain", null);
		t.addChild(message);
		t.addChild(messageApp);
