...
public class StateDelegate implements ReportDelegate<StateReport> {
    ...
    public boolean createHourlyTask(StateReport report) {
        ...
        m_taskManager.createTask(startTime, domain, StateAnalyzer.ID, TaskPolicy.ALL);
        m_taskManager.createTask(startTime, domain, Constants.REPORT_ROUTER, TaskPolicy.HOURLY);
        m_taskManager.createTask(startTime, domain, Constants.APP_DATABASE_PRUNER, TaskPolicy.DAILY);
        m_taskManager.createTask(startTime, domain, Constants.METRIC_GRAPH_PRUNER, TaskPolicy.DAILY);
//		m_taskManager.createTask(startTime, domain, Constants.WEB_DATABASE_PRUNER, TaskProlicy.DAILY);
        ...
        m_taskManager.createTask(startTime, domain, Constants.REPORT_DATABASE_CAPACITY, TaskPolicy.ALL);
        m_taskManager.createTask(startTime, domain, Constants.REPORT_JAR, TaskPolicy.HOURLY);
        m_taskManager.createTask(startTime, domain, Constants.REPORT_HEAVY, TaskPolicy.ALL);
        m_taskManager.createTask(startTime, domain, Constants.REPORT_UTILIZATION, TaskPolicy.ALL);
        m_taskManager.createTask(startTime, domain, Constants.REPORT_SERVICE, TaskPolicy.ALL);
        ...
        if (hour >= 4) {
            m_taskManager.createTask(startTime, domain, Constants.CURRENT_REPORT, TaskPolicy.DAILY);
            m_taskManager.createTask(startTime, domain, Constants.REPORT_CLIENT, TaskPolicy.DAILY);
        }
        // clear local report
