...
public class AlertManager implements Initializable {
    ...
    public List<AlertEntity> queryLatestAlarmKey(int minute) {
    ...
    private boolean send(AlertEntity alert) {
        ...
        for (AlertChannel channel : channels) {
            ...
            if (channel.isNeedReceivers() && (receivers == null || receivers.size() <= 0)) {
                Cat.logError("No receiver found for AlertChannel:" + channel, new Exception());
                continue;
            } else {
