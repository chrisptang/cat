...
public class HeartbeatAlert implements Task {
                    ...
                    if (minute >= maxMinute - 1) {
                        for (Machine machine : currentReport.getMachines().values()) {
                            ...
                            if (arguments != null) {
                                ...
                                processMetric(domain, ip, metric, conditions, maxMinute, values);
                            }
                        }
                    } else if (minute < 0) {
    ...
    private void processMetric(String domain, String ip, String metric, List<Condition> conditions, int maxMinute,
                               double[] values) {
        try {
            if (values != null) {
