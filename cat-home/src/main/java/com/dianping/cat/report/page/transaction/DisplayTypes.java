...
public class DisplayTypes {
    ...
    public static class TransactionTypeComparator implements Comparator<TransactionTypeModel> {
        ...
        public int compare(TransactionTypeModel m1, TransactionTypeModel m2) {
            if (null == m_sorted || m_sorted.length() <= 1) {
                //sort by failure than sort by total;
                int failureSorted = (int) (m2.getDetail().getFailCount() - m1.getDetail().getFailCount());
                if (failureSorted != 0) {
                    return failureSorted;
                } else {
